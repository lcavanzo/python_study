...
def do_search() -> 'html':
    ...
    def log_request(req: 'flask_request', res: str) -> None:
        ...
        with UseDatabase(app.config['dbconfig']) as cursor:
            _SQL = """INSERT INTO log (phrase, letters, ip, browser_string,
                results) values (%s, %s, %s, %s, %s)"""
...
def view_the_log() -> 'html':
    try:
        ...
        return render_template('view_log.html', the_title='View_Log',
                the_row_titles=titles,
            the_data=contents,)
    except ConnError as err:
        print('Is your database switched on?', str(err))
    except CredentialsError as err:
        print('User-ID/passwd issues. Error:', str(err))
...
app.secret_key = 'YouWillNeverGuessMyPass'
